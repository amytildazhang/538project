...
for (i in 1:length(polldata$Poll)) {
  ...
  print(paste(i, poll.ratings$Simple.Plus.Minus[poll.ratings$Pollster == pster]))
...
for (i in 1:length(polldata$Poll)){
  ...
  if (length(total.hfx != 0)) {
    ...
    polls$HXAdjusted[i] = total.hfx + polls$TAdjusted[i]
  }
}
...
for (i in 1:51){
  ...
  if (elect.votes$State[i] == 'DC') {
    ...
    statedata = polldata[polldata$State == elect.votes$State[i],]
    spread = statedata$HXAdjusted #CHANGE TO HFX ADJUSTED
    print(paste("spread", spread))
    pop.vote = mean(spread * statedata$Sampleweight * statedata$recency * statedata$rating)
    print(paste("pop vote", pop.vote))
    if (elect.votes$State[i] == 'MN' | elect.votes$State[i] == 'NE'){
      print("is MN or NE")
