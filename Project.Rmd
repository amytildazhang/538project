...
Nate Silver made waves in the 2008 election when he correctly forecasted the outcomes of 49 out of 50 of the states using statistics [intro stuff here]. I attempted to replicate his predictions for the 2012 election, using the methodology described on his blog [1]. 
...
In the time between the day a poll was taken and the election date, it is very likely that the political landscape will change (such as one candidate performing extremely well at a debate, thus shifting public opinion towards him). The trendline adjustment seeks to account for that by 
...
The house effects of the pollsters are determined using a linear regression. The model is $TAdjusted_i = \beta_0 + \beta_1*State_i + \beta_2*Pollster_i + \epsilon_i, \epsilon~N(0,\sigma^2)$, where TAdjusted is the poll's results, adjusted as described previously, and 
...
for (i in 1:length(polls$PIE)){
  ...
  polls$plus.min= polls$PIE[i] - polls$IAE[i]
} 
...
for (i in 1:51){
  if (elect.votes[i] == 'DC') {}
    ...
    statedata = polldata[polldata$State == elect.votes[i],]
    spread = statedata$TAdjusted #CHANGE TO HFX ADJUSTED
    pop.vote = mean(spread * statedata$Sampleweight * statedata$recency * statedata$ESS)
    if (elect.votes[i] == 'MN' | elect.votes[i] == 'NE'){
      #based on coefficients for MN, NE in housefx.fit, determine if red or blue state
      predicted.votes$votes[i] = round(pop.vote * elect.votes$votes[i])
    }
    else {
